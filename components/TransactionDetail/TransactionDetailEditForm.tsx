...
const TransactionDetailEditForm = ({
  setShowModal,
  transaction,
}: ModalState) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: z.infer<typeof UpdateTransactions>) => {
      const dateInfo = data.PurchaseDate ? data.PurchaseDate.getTime() : null;
      const transformData = {
        ...data,
  ...
  const form = useForm<z.infer<typeof UpdateTransactions>>({
    resolver: zodResolver(UpdateTransactions),
    defaultValues: {
  ...
  const handleSubmit = async (data: z.infer<typeof UpdateTransactions>) => {
    try {
  ...
  return (
    <>
      <main className="z-50 fixed inset-0 bg-richBlack/50 backdrop-blur-sm flex justify-center items-center p-4">
